...
int main()
{
	...
	double adis_data_ [ADIS_HD_NUMREGISTERS__];
	bool isOpened_spi;

	uint8_t adis_commands_[ADIS_HD_BUFFER_SIZE__]; // length of data buffer must be 11 entries x 2 byte = 22!!!!!
	uint8_t adis_rawdata_[ADIS_HD_BUFFER_SIZE__]; //length of data buffer must be 11 entries x 2 byte = 22!!!!!
	adis_hd_init(adis_commands_);

	BlackLib::BlackSPI adis_hd_spi_(BlackLib::SPI0_0, 8, BlackLib::SpiMode3, 300000); // Declare IMU device and open SPI port
	//BlackLib::BlackSPI adis_spi_(BlackLib::SPI0_0, 8, BlackLib::SpiMode3, 2000000);
	isOpened_spi = adis_hd_spi_.open(BlackLib::NonBlock);
	...
	rtc_set_alarm_i2c(&rtc_i2c_, stopalarmtime_,1, isOpened_i2c); // set second alarm clock which is actually nonsense because we never check for it again, because TOO SLOW!
	...
	cout << "I have set the following alarm: " << controlalarm_[0] << "--" << controlalarm_[1] << ":" << controlalarm_[2] << ":" << controlalarm_[3] << endl;
		rtc_data_[5] = (double)startalarmtime_[3]; // Here we save the startup day, hours and minutes
	...
	rtc_display(rtc_data_); // Show the current time

	// STARTING THE PROGRAM LOOP
	cout << "I am the HD version designed for ADIS16488" << endl;
	...
	while ( !stopalarm_)
	{
		...
		// Read the raw data
		adis_hd_read_spi(&adis_hd_spi_, adis_commands_, adis_rawdata_, isOpened_spi); // Read raw ADIS data
		rtc_read_i2c(&rtc_i2c_, rtc_rawdata_, isOpened_i2c); // Read raw Real Time Clock data
		...
		adis_hd_extract_message(adis_rawdata_, adis_data_); // Convert weird ADIS data to readable data
		rtc_extract_message(rtc_rawdata_, rtc_data_); // Convert RTC data to readable data
		...
		if (rtc_data_[5] - prevsec != 0) { // FIX THAT WHEN TIME!! I am giving up, I am hacking my way through here. We only read seconds from the RTC and improvise the rest now
			prevsec = rtc_data_[5]; // This monster-hack works because before we saved the startup day, hour and minute from the alarm oh god I hate myself for this code
			if (rtc_data_[5] == 0) {
				rtc_data_[4]++;
				if (rtc_data_[4] > 60) {
					rtc_data_[4] = 0;
		...
		// Display data
		//adis_hd_display(adis_data_);
		//rtc_display(rtc_data_);

		// Send data to CSV file
		csvout->csv_hd_write(readcount, adis_data_,rtc_data_);

		//stopalarm_ = rtc_check_alarm (&rtc_i2c_, 1, isOpened_i2c);
		stopalarm_ = rtc_check_stopalarm(stopalarmtime_,rtc_data_); // we check the stopalarm against the system clock, not RTC, because RTC is TOO SLOW
	...
	adis_hd_spi_.close(); // Close SPI connection to IMU
